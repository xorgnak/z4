...
class Ical2Rem
  ...
  def initialize f
    @rem = []
    parse(RiCal.parse_string(File.read(f).strip).first)
  end
  ...
  def parse(cal)
    ...
    cal.events.each do |event|
      ...
      # REM Jan 01 2008
      @rem << "REM #{vstart.strftime("%b")} #{vstart.day} #{vstart.year}"
      ...
      if event.bounded? && (event.occurrences.length > 1)
        ...
        if not same_day?(vstart, last)
          rem << " UNTIL #{last.strftime("%b")} #{last.day} #{vend.year}"
        end
      end
      ...
      if is_datetime
        rem << " AT #{vstart.strftime("%H:%M")}"
        rem << " DURATION #{duration / 3600}:#{(duration % 3600) / 60}"
      end
      ...
      rem << " at #{event.location}" if event.location
      @rem << rem.join("")
    end
    cal.todos.each do |todo|
      ...
      if todo.status and todo.status == "COMPLETED"
        ...
        @rem << "REM #{due.strftime("%b")} #{due.day} #{due.year} #{lead} #{priority} MSG \%a %\"#{todo.summary}\%\"\%\"\%"
      end
    end
  end
...
module Z4
  @@REM = Hash.new { |h,k| h[k] = Ical2Rem.new(k) }
  def self.remind
    @@REM
  end
  def self.reminders x, c, *p
    File.open("rem/#{x}.rem") { |f| f.write([@@REM[c].to_rem, p].flatten.join("\n")); }
  end
  def self.calendar x
    `remind -pp rem/#{x}.rem | rem2html`
  end
  def self.agenda x
    `remind -t1 rem/#{x}.rem`
  end
