...
WIKI["Downtown Denver, Colorado"]
WIKI["Capitol Hill, Denver, Colorado"]
...
Z4.chan[:purpose] = "What is this channel used for?\nRespond '##purpose PURPOSE' to set it."
...
["whiskey","vodka","gin","absenthe","cocktails","wine","beer","pool","darts","food","skiball","hoops","airhockey","thering","door","floor"].each { |e| TAG.safe "bar", e }

["best","86","cool","dance","pub"].each { |e| TAG.award "bar", e }

["8ball","9ball"].each { |e| TAG.safe "pool", e  }

["bar","area","city"].each { |e| TAG.award "pool", e }

["pizza","pasta","sandwich","burger","gyro","fries","eggs","burritos","tacos","vegan","vegetarian","8ball"].each { |e| TAG.safe "food", e }

["best","late","fast"].each { |e| TAG.award "food", e }

["flower","wax","cavier","rosin","edibles","hashish"].each { |e| TAG.safe "weed", e  }

["area","city"].each { |e| TAG.award "weed", e }

["bar","food","door","sandwich","weed"].each {|e| TAG.safe "local", e }

TAG.award "local","best"
